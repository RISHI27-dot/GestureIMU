...
const unsigned char hidapiReportMap[] = {
	...
	0x82,
	0x02,
	0x01, //   Input (Data,Var,Abs,No Wrap,Linear,Preferred State,No Null Position,Buffered Bytes)
	0x95,
	0x08, //   Report Count (8)
	0x09,
	0x02, //   Usage (0x02)
...
esp_err_t ble_hid_task(int16_t *data)
{
	...
	ESP_LOGI(TAG, "Available int16_t data: %d %d %d", data[0], data[1],
		 data[2]);
	ESP_LOGI(TAG, "Available int16_t data: %x %x %x", data[0], data[1],
		 data[2]);

	// TODO : what we are doing is reading two 8 bit values from MPU6050 and combining them
	// to form a 16 bit value and then again breaking them to be transmitted via BLE. For now
	// let this overhead be...later send the values via BLE without fusion itself.

	//////////////////// This will go //////////////////////////////////////////
	for (i = 0, j = 0; i < 3; i++, j += 2) {
		data_to_send[j] = data[i] >> 8;
		data_to_send[j + 1] = data[i];
	}
	////////////////////////////////////////////////////////////////////////////

	ESP_LOGI(TAG, "Sending broken uint8_t data: %x %x %x %x %x %x",
		 data_to_send[0], data_to_send[1], data_to_send[2],
		 data_to_send[3], data_to_send[4], data_to_send[5]);

	ret = esp_hidd_dev_input_set(s_ble_hid_param.hid_dev, 1,
				     HID_RPT_ID_CC_IN, data_to_send,
				     sizeof(data_to_send));
...
void ble_hidd_event_callback(void *handler_args, esp_event_base_t base,
			     int32_t id, void *event_data)
{
	...
	case ESP_HIDD_PROTOCOL_MODE_EVENT: {
		ESP_LOGI(TAG, "PROTOCOL MODE[%u]: %s",
			 ...
			 param->protocol_mode.protocol_mode ? "REPORT" :
							      "BOOT");
		break;
	}
...
void setup_ble(void)
{
	...
	if ((ret = esp_ble_gatts_register_callback(
	...
	ESP_ERROR_CHECK(esp_hidd_dev_init(
		&ble_hid_config, ESP_HID_TRANSPORT_BLE, ble_hidd_event_callback,
