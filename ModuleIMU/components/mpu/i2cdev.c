...
esp_err_t i2cdev_init()
{
	if (!i2c_initialised) {
		...
		for (int i = 0; i < I2C_NUM_MAX; i++) {
			...
			if (!states[i].lock) {
				ESP_LOGE(TAG, "Could not create port mutex %d",
					 i);
				return ESP_FAIL;
			}
		}
...
esp_err_t i2c_dev_create_mutex(i2c_dev_t *dev)
{
	...
	if (!dev->mutex) {
		ESP_LOGE(TAG, "[0x%02x at %d] Could not create device mutex",
			 dev->addr, dev->port);
		return ESP_FAIL;
	}
#endif
...
esp_err_t i2c_dev_take_mutex(i2c_dev_t *dev)
{
	...
	if (!xSemaphoreTake(dev->mutex, pdMS_TO_TICKS(CONFIG_I2CDEV_TIMEOUT))) {
		ESP_LOGE(TAG, "[0x%02x at %d] Could not take device mutex",
			 dev->addr, dev->port);
		return ESP_ERR_TIMEOUT;
	}
#endif
...
esp_err_t i2c_dev_give_mutex(i2c_dev_t *dev)
{
	...
	if (!xSemaphoreGive(dev->mutex)) {
		ESP_LOGE(TAG, "[0x%02x at %d] Could not give device mutex",
			 dev->addr, dev->port);
		return ESP_FAIL;
	}
#endif
...
static esp_err_t i2c_setup_port(const i2c_dev_t *dev)
{
	...
	if (!cfg_equal(&dev->cfg, &states[dev->port].config)) {
		...
		if ((res = i2c_param_config(dev->port, &temp)) != ESP_OK)
			return res;
		if ((res = i2c_driver_install(dev->port, temp.mode, 0, 0, 0)) !=
		    ESP_OK)
			return res;

		states[dev->port].installed = true;
		memcpy(&states[dev->port].config, &temp, sizeof(i2c_config_t));
		ESP_LOGD(TAG, "I2C driver successfully reconfigured on port %d",
			 dev->port);
	}
	...
	if ((res = i2c_get_timeout(dev->port, &t)) != ESP_OK)
		return res;
	// Timeout cannot be 0
	uint32_t ticks = dev->timeout_ticks ? dev->timeout_ticks :
					      I2CDEV_MAX_STRETCH_TIME;
	if ((ticks != t) && (res = i2c_set_timeout(dev->port, ticks)) != ESP_OK)
		return res;
	ESP_LOGD(TAG, "Timeout: ticks = %lu (%lu usec) on port %d",
		       ...
		       size_t out_size, void *in_data, size_t in_size)
{
	...
	if (res == ESP_OK) {
		...
		if (res != ESP_OK)
			ESP_LOGE(
				TAG,
				"Could not read from device [0x%02x at %d]: %d (%s)",
			...
			size_t out_size)
{
	...
	if (res == ESP_OK) {
		...
		if (out_reg && out_reg_size)
			i2c_master_write(cmd, (void *)out_reg, out_reg_size,
					 true);
		i2c_master_write(cmd, (void *)out_data, out_size, true);
		i2c_master_stop(cmd);
		res = i2c_master_cmd_begin(
		...
		if (res != ESP_OK)
			ESP_LOGE(
				TAG,
				"Could not write to device [0x%02x at %d]: %d (%s)",
				dev->addr, dev->port, res,
				esp_err_to_name(res));
		i2c_cmd_link_delete(cmd);
	}
