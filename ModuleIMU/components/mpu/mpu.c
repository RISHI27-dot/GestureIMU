...
esp_err_t i2c_master_init(void)
{
	...
	mpu_dev_t.port = I2C_MASTER_NUM;
	mpu_dev_t.addr = mpu_ADDR;
	mpu_dev_t.cfg.mode = I2C_MODE_MASTER;
	mpu_dev_t.cfg.sda_io_num = I2C_MASTER_SDA_IO;
	mpu_dev_t.cfg.scl_io_num = I2C_MASTER_SCL_IO;
	mpu_dev_t.cfg.master.clk_speed = I2C_MASTER_FREQ_HZ;

	return i2c_dev_create_mutex(&mpu_dev_t);
}

// Initialise and power ON, mpu
esp_err_t enable_mpu(void)
{
	CHECK(i2c_master_init());
	I2C_DEV_TAKE_MUTEX(&mpu_dev_t);

	// Set Clock Source
	uint8_t value;
	I2C_DEV_CHECK(&mpu_dev_t,
		      i2c_dev_read_reg(&mpu_dev_t, mpu_RA_PWR_MGMT_1, &value,
				       1));
	value = value & 0xf8;
	value = value | mpu_CLOCK_PLL_XGYRO;
	I2C_DEV_CHECK(&mpu_dev_t,
		      i2c_dev_write_reg(&mpu_dev_t, mpu_RA_PWR_MGMT_1, &value,
					1));

	// Set fullscale gyro range
	value = mpu_GYRO_FS_250;
	I2C_DEV_CHECK(&mpu_dev_t,
		      i2c_dev_write_reg(&mpu_dev_t, mpu_RA_GYRO_CONFIG, &value,
					1));

	// Set fullscale accel range
	value = mpu_ACCEL_FS_2;
	I2C_DEV_CHECK(&mpu_dev_t,
		      i2c_dev_write_reg(&mpu_dev_t, mpu_RA_ACCEL_CONFIG, &value,
					1));
	...
	I2C_DEV_GIVE_MUTEX(&mpu_dev_t);

	return ESP_OK;
}

// Read accelerometer values
esp_err_t mpu_read_acce(uint8_t *data_rd, size_t size)
{
	if (size == 0) {
		return ESP_OK;
	}
	...
	I2C_DEV_TAKE_MUTEX(&mpu_dev_t);
	I2C_DEV_CHECK(&mpu_dev_t, i2c_dev_read_reg(&mpu_dev_t, ACCE_START_ADDR,
						   data_rd, size));
	I2C_DEV_GIVE_MUTEX(&mpu_dev_t);
...
esp_err_t mpu_read_gyro(uint8_t *data_rd, size_t size)
{
	if (size == 0) {
		return ESP_OK;
	}
	...
	I2C_DEV_TAKE_MUTEX(&mpu_dev_t);
	I2C_DEV_CHECK(&mpu_dev_t, i2c_dev_read_reg(&mpu_dev_t, GYRO_START_ADDR,
						   data_rd, size));
	I2C_DEV_GIVE_MUTEX(&mpu_dev_t);
...
esp_err_t read_mpu_raw(int16_t *acce_raw_value, int16_t *gyro_raw_value)
{
	esp_err_t err = ESP_FAIL;
	uint8_t acce_rd[BUFF_SIZE], gyro_rd[BUFF_SIZE];

	if (mpu_read_acce(acce_rd, BUFF_SIZE) == ESP_OK &&
	    mpu_read_gyro(gyro_rd, BUFF_SIZE) == ESP_OK)
		err = ESP_OK;
			  ...
			  float *complementary_angle, float *mpu_offset)
{
	...
	if (is_initial_reading) {
		...
		compute_acce_angle(acce_raw_value[0], acce_raw_value[1],
				   acce_raw_value[2], acce_angle);
	...
	compute_acce_angle(acce_raw_value[0], acce_raw_value[1],
			   acce_raw_value[2], acce_angle);
	compute_gyro_angle(gyro_raw_value[0], gyro_raw_value[1],
			   gyro_raw_value[2], dt, gyro_angle);

	for (i = 0; i < 2; i++) {
		acce_angle[i] = acce_angle[i] - mpu_offset[i];
		complementary_angle[i] =
			ALPHA * (complementary_angle[i] + gyro_angle[i]) +
			(1 - ALPHA) * acce_angle[i];
	}
}

esp_err_t read_mpu(float *euler_angle, float *mpu_offset)
{
	...
	complementary_filter(acce_raw_value, gyro_raw_value,
			     complementary_angle, mpu_offset);
